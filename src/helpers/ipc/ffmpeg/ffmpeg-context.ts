...
export function exposeFfmpegContext() {
  contextBridge.exposeInMainWorld("ffmpegAPI", {
    ...
    onDownloadProgress: (callback: (progress: any) => void) => {
      ...
      return () =>
        ipcRenderer.removeListener(FFMPEG_CHANNELS.DOWNLOAD_PROGRESS, listener);
    },
    onDownloadComplete: (callback: () => void) => {
      const listener = () => callback();
      ipcRenderer.on(FFMPEG_CHANNELS.DOWNLOAD_COMPLETE, listener);
      return () =>
        ipcRenderer.removeListener(FFMPEG_CHANNELS.DOWNLOAD_COMPLETE, listener);
    },
    ...
    onProgress: (callback: (progress: FFmpegProgress) => void) => {
      ...
      return () =>
        ipcRenderer.removeListener(FFMPEG_CHANNELS.PROGRESS_UPDATE, listener);
    },
    onLog: (callback: (data: { log: string; type: string }) => void) => {
      const listener = (_event: any, data: { log: string; type: string }) =>
        callback(data);
      ipcRenderer.on(FFMPEG_CHANNELS.LOG_OUTPUT, listener);
      return () =>
        ipcRenderer.removeListener(FFMPEG_CHANNELS.LOG_OUTPUT, listener);
    },
    onComplete: (callback: (outputPath: string) => void) => {
      const listener = (_event: any, outputPath: string) =>
        callback(outputPath);
      ipcRenderer.on(FFMPEG_CHANNELS.PROCESS_COMPLETE, listener);
      return () =>
        ipcRenderer.removeListener(FFMPEG_CHANNELS.PROCESS_COMPLETE, listener);
    },
    onError: (callback: (error: string) => void) => {
      const listener = (_event: any, error: string) => callback(error);
      ipcRenderer.on(FFMPEG_CHANNELS.PROCESS_ERROR, listener);
      return () =>
        ipcRenderer.removeListener(FFMPEG_CHANNELS.PROCESS_ERROR, listener);
    },
    ...
    onQueueUpdate: (callback: (queue: any[]) => void) => {
      ...
      return () =>
        ipcRenderer.removeListener(FFMPEG_CHANNELS.QUEUE_UPDATE, listener);
    },
    onQueueItemUpdate: (callback: (item: any) => void) => {
      ...
      return () =>
        ipcRenderer.removeListener(FFMPEG_CHANNELS.QUEUE_ITEM_UPDATE, listener);
    },
    onQueueItemProgress: (
      callback: (data: { itemId: string; progress: any }) => void,
    ) => {
      const listener = (_event: any, data: { itemId: string; progress: any }) =>
        callback(data);
      ipcRenderer.on(FFMPEG_CHANNELS.QUEUE_ITEM_PROGRESS, listener);
      return () =>
        ipcRenderer.removeListener(
          FFMPEG_CHANNELS.QUEUE_ITEM_PROGRESS,
          listener,
        );
    },
    onQueueItemLog: (
      ...
      const listener = (
        _event: any,
        data: { itemId: string; log: string; type: string },
      ) => callback(data);
      ipcRenderer.on(FFMPEG_CHANNELS.QUEUE_ITEM_LOG, listener);
      return () =>
        ipcRenderer.removeListener(FFMPEG_CHANNELS.QUEUE_ITEM_LOG, listener);
    },
    onQueueItemComplete: (
      ...
      const listener = (
        _event: any,
        data: { itemId: string; outputPath: string },
      ) => callback(data);
      ipcRenderer.on(FFMPEG_CHANNELS.QUEUE_ITEM_COMPLETE, listener);
      return () =>
        ipcRenderer.removeListener(
          FFMPEG_CHANNELS.QUEUE_ITEM_COMPLETE,
          listener,
        );
    },
    onQueueItemError: (
      ...
      const listener = (_event: any, data: { itemId: string; error: string }) =>
        callback(data);
      ipcRenderer.on(FFMPEG_CHANNELS.QUEUE_ITEM_ERROR, listener);
      return () =>
        ipcRenderer.removeListener(FFMPEG_CHANNELS.QUEUE_ITEM_ERROR, listener);
    },
    onQueueComplete: (callback: () => void) => {
      const listener = () => callback();
      ipcRenderer.on(FFMPEG_CHANNELS.QUEUE_COMPLETE, listener);
      return () =>
        ipcRenderer.removeListener(FFMPEG_CHANNELS.QUEUE_COMPLETE, listener);
    },
  });
}
